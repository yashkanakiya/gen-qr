  ...
  <div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
      <div class="text-center mb-8">
        <div
          class="w-16 h-16 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg"
        >
      ...
      <div class="space-y-5">
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">
            Give your QR code a memorable name (min 3 chars, no spaces)
          </p>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-50 to-white rounded-lg p-6">
          ...
          <div class="flex justify-center mb-6">
            <div class="bg-white p-4 rounded-xl shadow-md border border-gray-200">
              <img ref="qrImage" :src="qrSrc" :alt="form.name" class="w-48 h-48 object-contain" />
            </div>
          </div>
          ...
          <!-- Download Options -->
          <div class="mb-6">
            <label class="block text-sm font-semibold text-gray-700 mb-3"> Download Options </label>
            <div class="grid grid-cols-3 gap-3">
          ...
          <div class="flex flex-col gap-3">
            <Button label="Save" severity="success" @click="openSaveModal" class="w-full" />
            <Button label="Create Another" severity="secondary" @click="resetForm" outlined />
          </div>
        </div>
      </div>
    </div>
      ...
      <template #footer>
        <div class="flex gap-3">
          <Button
            ...
            class="flex-1"
          />
          <Button label="Save" severity="success" @click="saveToDashboard" class="flex-1" />
        </div>
      </template>
    </Dialog>
...
import { saveQRCode } from '../stores/qrStore' // ← This now uses API
...
// UPDATED: Save to backend API instead of localStorage
async function saveToDashboard() {
  isLoading.value = true

  try {
    // Only send name and url - QR code will be generated by backend
    await saveQRCode({
      name: form.name,
      url: form.url,
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Save Failed',
      detail: error.message || 'Failed to save QR code. Please try again.',
      life: 4000,
    })
  } finally {
    isLoading.value = false
